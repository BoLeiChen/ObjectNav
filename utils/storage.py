...
class RolloutStorage_3d(object):
    ...
    def __init__(self, num_steps, num_processes, obs_shape, points_obs_shape, action_space,
                 rec_state_size):
    ...
    def insert(self, obs_, obs_points, rec_states, actions, action_log_probs, value_preds,
               rewards, masks):
        self.obs_[self.step + 1].copy_(obs_)
        self.obs_points[self.step + 1].copy_(obs_points)
        self.actions[self.step].copy_(actions.view(-1, self.n_actions))
        self.action_log_probs[self.step].copy_(action_log_probs)
        self.value_preds[self.step].copy_(value_preds)
    ...
    def feed_forward_generator(self, advantages, num_mini_batch):
        ...
        for indices in sampler:
            yield {
                'obs_' : self.obs_[:-1].view(-1, *self.obs_.size()[2:])[indices],
                'obs_points': self.obs_points[:-1].view(-1, *self.obs_points.size()[2:])[indices],
                'rec_states': self.rec_states[:-1].view(
                    -1, self.rec_states.size(-1))[indices],
                'actions': self.actions.view(-1, self.n_actions)[indices],
...
class Identify_GlobalRolloutStorage(RolloutStorage_3d):
    ...
    def __init__(self, num_steps, num_processes, obs_shape, obs_points_shape, action_space,
                 rec_state_size, extras_size):
        super(Identify_GlobalRolloutStorage, self).__init__(
            num_steps, num_processes, obs_shape, obs_points_shape, action_space, rec_state_size)
        self.extras = torch.zeros((num_steps + 1, num_processes, extras_size),
                                  dtype=torch.long)
        self.has_extras = True
        self.extras_size = extras_size
    ...
    def insert(self, obs_, obs_points, rec_states, actions, action_log_probs, value_preds,
        ...
        super(Identify_GlobalRolloutStorage, self).insert(
            obs_, obs_points, rec_states, actions,
            action_log_probs, value_preds, rewards, masks)

class RolloutStorage_IAM(object):
...
class GlobalRolloutStorage(RolloutStorage_IAM):

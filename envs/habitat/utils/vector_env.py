...
class VectorEnv:
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                ...
                # TODO: update CALL_COMMAND for getting attribute like this
                    ...
                    observations, obs_, reward, done, info = \
                            env.plan_act_and_preprocess(data)
                    if auto_reset_done and done:
                        observations, obs_, info = env.reset()
                    connection_write_fn((observations, obs_, reward, done, info))
    ...
    def reset(self):
        ...
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        obs, obs_, infos = zip(*results)

        self._is_waiting = False
        return np.stack(obs), np.stack(obs_), infos
    ...
    def plan_act_and_preprocess(self, inputs):
        ...
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        obs, obs_, rews, dones, infos = zip(*results)
        self._is_waiting = False
        return np.stack(obs), np.stack(obs_), np.stack(rews), np.stack(dones), infos

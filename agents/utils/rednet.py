...
class SemanticPredRedNet():
    ...
    def get_prediction(self, img, depth, cat_goal):
        ...
        output, mask = self.segmentation_model(img,depth) # output: (bs, 40, 480, 640)
        output = output[0] # 40*480*640


        #=================entropy================
        semantic_pred = torch.nn.functional.softmax(output, dim=0).permute(1,2,0).cpu().numpy() #(480, 640, 40)
        entropy_tmp = -semantic_pred*np.log(semantic_pred) #(480, 640, 40)
        sem_entropy = np.sum(entropy_tmp, axis=2) # (480, 640)

        output = torch.nn.functional.softmax(output, dim=0).permute(1,2,0).cpu().numpy() #(480, 640, 40)
        output[np.where(np.max(output, axis=-1) < self.threshold)] = 0
        
        if args.dataset == "mp3d":
            semantic_input = np.zeros((img.shape[1], img.shape[2], len(mp3d_habitat_labels) )) #(480, 640, 22)
        ...
        #=================prob points================
        goal_cat_output = semantic_input[:, :, cat_goal] + 1e-5 # (480, 640)
